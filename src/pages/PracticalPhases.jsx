  ...
  return (
    <>
      <PageHero
        ...
        intro="Reading about a job and doing it for a day are completely different experiences. This stage is about getting real information from real workplaces."
      />

      <section className="mx-auto max-w-3xl px-5 py-14 md:px-8 md:py-20">
        <h2 className="font-display text-xl font-semibold text-ink">Ways to get hands-on experience</h2>
        <div className="mt-6 space-y-4">
          {phases.map((p) => (
            <div key={p.title} className="flex flex-col gap-1 rounded-xl border border-paper-line p-6 sm:flex-row sm:items-start sm:justify-between sm:gap-6">
              <div>
              ...
              <span className="mt-2 inline-block shrink-0 rounded-full bg-forest/10 px-3 py-1 font-mono text-xs text-forest sm:mt-0">
                {p.length}
              </span>
            </div>
          ))}
        ...
        <p className="mt-3 leading-relaxed text-slate">
          The placement itself is only half the value — what you write down
          afterward is what actually informs your next decision. Use the
          reflection log from the documents page to capture what surprised
          you, what you'd want more of, and what you'd want to avoid.
        </p>

        <div className="mt-10">
          <Callout label="Tip">
            A placement that confirms a job isn't for you is just as useful
            as one that confirms it is. Both are progress.
          </Callout>
        </div>
      </section>
